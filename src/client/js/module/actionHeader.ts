...
export const bindMainMenuButton = () => {
  const mainMenuButton = header.querySelector(
    '.btn-main-menu',
...
export const setHeaderColorByScroll = (mainSection: HTMLElement) => {
  ...
  let colorRGB =
    255 - (document.documentElement.scrollTop / mainSection.offsetHeight) * 255;
...
const actionToggleMainMenu = {
  ...
  actionMenuOpen: function (
    btn: { classList: { add: (arg0: string) => void } },
    menu: { classList: { add: (arg0: string) => void } },
  ): void {
  ...
  actionMenuClose: function (
    btn: { classList: { remove: (arg0: string) => void } },
    menu: { classList: { remove: (arg0: string) => void } },
  ): void {
