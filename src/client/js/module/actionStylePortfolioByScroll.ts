...
const categoryTab = portfolioSection.querySelector(
  '.category-tab-wrap',
) as HTMLElement;
...
export const activePortfolioByScroll = {
  ...
  tabMotion: {
    elementRelativeTop:
      window.pageYOffset + categoryTab.getBoundingClientRect().top,
    endPoint: portfolioSection.offsetTop + portfolioSection.scrollHeight,
    action(scrollBottom: number) {
      if (
        scrollBottom - window.innerHeight > this.elementRelativeTop &&
  ...
  setImgActive: {
    pc(scrollBottom: number, i: number, arr: NodeListOf<Element>): void {
      ...
      if (target) {
        ...
        let startPoint =
          window.pageYOffset + arr[i].getBoundingClientRect().top <
          scrollBottom;
        if (startPoint) {
